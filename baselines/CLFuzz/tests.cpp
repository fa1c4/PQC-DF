...
void test(const operation::ECC_ValidatePubkey& op, const std::optional<bool>& result) {
    ...
    if ( result != std::nullopt ) {
        if(*result == true){
            if(op.id >= 2 && op.id <= 4){
                std::cout << "result should be False but get True" << std::endl;
                ::abort();
            }
        } else {
            if(op.id == 1){
                std::cout << "result should be True but get False" << std::endl;
                ::abort();
            }
        }
    }
...
void test(const operation::ECDSA_Verify& op, const std::optional<bool>& result) {
    if( result != std::nullopt){
        if(*result == true){
            if(op.id >= 2 && op.id <= 4){
                if(!(op.digestType == CF_DIGEST("SHA256") 
                // && (
                //    op.curveType == CF_ECC_CURVE("secp160k1")||
                //    op.curveType == CF_ECC_CURVE("secp160r1")||
                //    op.curveType == CF_ECC_CURVE("secp160r2")||
                //    op.curveType == CF_ECC_CURVE("secp224k1")||
                //    op.curveType == CF_ECC_CURVE("x962_p239v1")||
                //    op.curveType == CF_ECC_CURVE("x962_p239v2")||
                //    op.curveType == CF_ECC_CURVE("x962_p239v3")
                // )
                )){
                    std::cout << "result should be False but get True" << std::to_string(op.id) << std::endl;
                    ::abort();
                }
            }
        } else {
            if(op.id == 1){
                ...
                // )
                    ...
                    std::cout << "result should be True but get False" << std::to_string(op.id) << std::endl;
                    ::abort();
                }
            }
        }
...
void test(const operation::BLS_Verify& op, const std::optional<bool>& result) {
    if( result != std::nullopt){
        if(*result == true){
            if(op.expected >= 2 && op.expected <= 3){
                std::cout << "result should be False but get True" << std::to_string(op.expected) << std::endl;
                ::abort();
            }
        } else {
            if(op.expected == 1){
                std::cout << "result should be True but get False" << std::to_string(op.expected) << std::endl;
                ::abort();
            }
        }
    }
...
void test(const operation::BLS_Compress_G1& op, const std::optional<component::Bignum>& result) {
    ...
    if( result != std::nullopt){
        if(op.expected >= 2 && op.expected <= 3){
            if(result->ToString() == op.compressed.ToString()){
                std::cout << "result should be False but get True" << std::to_string(op.expected) << std::endl;
                ::abort();
            }
        } else if(op.expected == 1){
            if(result->ToString() != op.compressed.ToString()){
                std::cout << "result should be True but get False" << std::to_string(op.expected) << std::endl;
                ::abort();
            }
...
void test(const operation::BLS_Compress_G2& op, const std::optional<component::G1>& result) {
    ...
    if( result != std::nullopt){
        if(op.expected >= 2 && op.expected <= 3){
            if(result->first.ToString() == op.compressed.first.ToString() || result->second.ToString() == op.compressed.second.ToString() ){
                std::cout << "result should be False but get True" << std::to_string(op.expected) << std::endl;
                ::abort();
            }
        } else if(op.expected == 1){
